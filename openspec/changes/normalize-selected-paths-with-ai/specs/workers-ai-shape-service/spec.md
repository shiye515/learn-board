...
- **WHEN** 检查生产构建产物
- **THEN** 客户端包中 SHALL 不包含 Cloudflare 绑定访问、模型提示、`guided_json` 配置和服务端推理实现
...
服务 SHALL 直接通过原生 `env.AI.run` 和模型原生 `guided_json` 获取结构化输出。第一阶段 SHALL 使用扁平分类 Schema，第二阶段 SHALL 仅在分类为 `common-symbol` 时请求矢量模板；服务对两个阶段执行严格 Zod 校验后，返回 `known-shape`、`common-symbol` 或 `unsupported` 可辨识结果。
...
- **WHEN** 服务处理图形标准化请求
- **THEN** 服务 SHALL 通过 Workers AI `AI` 绑定调用 `@cf/mistralai/mistral-small-3.1-24b-instruct`，且 SHALL 不在运行时静默切换到其他模型

#### Scenario: 模型接收多模态选区输入

- **WHEN** 服务调用 Mistral Small 3.1
...
#### Scenario: 模型省略可识别结果的置信度

- **WHEN** 第一阶段返回 `recognizable: true` 和有效已知图形分类，但省略 `confidence`
- **THEN** 服务 SHALL 使用 `0.9` 作为公开结果置信度，并继续执行标准化

#### Scenario: 常用符号触发第二阶段

- **WHEN** 第一阶段将输入分类为 `common-symbol`
- **THEN** 服务 SHALL 进行且仅进行一次额外 AI 调用以生成受约束矢量模板；已知图形 SHALL 不触发第二阶段

#### Scenario: 模型无法满足结构化输出

- **WHEN** Mistral Small 3.1 无法生成符合 `guided_json` 和 Zod Schema 的完整非流式结果
- **THEN** 服务 SHALL 返回类型化无效结果，且 SHALL 不使用未校验文本、自动重试或调用备用模型

### Requirement: 服务故障必须类型化且有界

服务 SHALL 将绑定不可用、超时、限流、平台和校验故障转换为类型化结果，并 SHALL 不向浏览器暴露平台堆栈信息或密钥。
...
- **WHEN** 校验、限流或推理失败
- **THEN** 日志 SHALL 包含请求 ID、延迟和分类错误代码，但不包含原始载荷、预览图或模型原始响应内容
