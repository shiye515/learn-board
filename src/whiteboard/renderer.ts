...
import type { Point, SelectionState, ViewportTransform, WhiteboardDocument, WhiteboardElement } from './model'
...
function drawStroke(ctx: CanvasRenderingContext2D, points: Point[], viewport: ViewportTransform, color: string, width: number) {
...
function drawNote(ctx: CanvasRenderingContext2D, element: Extract<WhiteboardElement, { kind: 'note' }>, viewport: ViewportTransform) {
  ...
  lines.forEach((line, index) => ctx.fillText(line, topLeft.x + 10 * viewport.scale, topLeft.y + (21 + index * 19) * viewport.scale))
}
...
function drawSelection(ctx: CanvasRenderingContext2D, document: WhiteboardDocument, selection: SelectionState, viewport: ViewportTransform) {
  ...
  if (elements.length) {
    ...
    ctx.lineWidth = 1
    ctx.strokeRect(topLeft.x, topLeft.y, (bounds.maxX - bounds.minX) * viewport.scale, (bounds.maxY - bounds.minY) * viewport.scale)
    ctx.restore()
  }
  if (selection.marquee) {
    const topLeft = worldToScreen({ x: selection.marquee.minX, y: selection.marquee.minY }, viewport)
    ctx.save()
    ctx.fillStyle = 'rgba(0,51,170,.10)'
    ctx.strokeStyle = '#0033aa'
    ctx.setLineDash([5, 4])
    ctx.fillRect(topLeft.x, topLeft.y, (selection.marquee.maxX - selection.marquee.minX) * viewport.scale, (selection.marquee.maxY - selection.marquee.minY) * viewport.scale)
    ctx.strokeRect(topLeft.x, topLeft.y, (selection.marquee.maxX - selection.marquee.minX) * viewport.scale, (selection.marquee.maxY - selection.marquee.minY) * viewport.scale)
    ctx.restore()
  }
}
...
export function drawWhiteboard(canvas: HTMLCanvasElement, document: WhiteboardDocument, viewport: ViewportTransform, selection: SelectionState, transientStroke?: { points: Point[], color: string, width: number }, ratio = 1) {
  ...
  for (const element of document.elements) {
    if (element.kind === 'stroke') drawStroke(ctx, element.points, viewport, element.color, element.width)
    else drawNote(ctx, element, viewport)
  }
  if (transientStroke) drawStroke(ctx, transientStroke.points, viewport, transientStroke.color, transientStroke.width)
