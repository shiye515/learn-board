...
describe('local whiteboard storage', () => {
  ...
  it('falls back to localStorage when IndexedDB is unavailable', async () => {
    const document = { ...createEmptyDocument(), elements: [{ kind: 'note' as const, id: 'note', x: 1, y: 2, width: 100, height: 40, text: 'saved', createdAt: 1 }] }
    const board = createPersistedBoard(document, { x: 10, y: 20, scale: 2 }, '#222222', 3)
    expect(await saveBoard(board)).toBe('saved')
    expect(JSON.parse(window.localStorage.getItem(STORAGE_KEY) ?? '{}').document.elements).toHaveLength(1)
