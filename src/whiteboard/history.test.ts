...
describe('whiteboard history', () => {
  it('undoes and redoes snapshots', () => {
    ...
    const after = { ...before, elements: [{ kind: 'note' as const, id: 'a', x: 0, y: 0, width: 100, height: 40, text: 'hello', createdAt: 1 }] }
  ...
  it('clears redo branches after a new commit', () => {
    ...
    const one = { ...empty, elements: [{ kind: 'note' as const, id: 'a', x: 0, y: 0, width: 100, height: 40, text: 'a', createdAt: 1 }] }
    const two = { ...empty, elements: [{ kind: 'note' as const, id: 'b', x: 0, y: 0, width: 100, height: 40, text: 'b', createdAt: 2 }] }
  ...
  it('caps history at 100 entries', () => {
    ...
    for (let i = 0; i < 101; i++) {
      const next = { ...previous, elements: [{ kind: 'note' as const, id: String(i), x: i, y: i, width: 100, height: 40, text: String(i), createdAt: i }] }
